...
csrf【跨站请求伪造】:用户登录站点后，不小心进入其他钓鱼网站，点击钓鱼网站的链接【链接跳转用户登录的站点并执行相应操作】
     ...
     所以这个时候，给登陆过的站点的cookie设置cookie的sameSite属性，防止其他页面发起对应服务端请求的时候，
     浏览器自动带上哪个服务器网站需要的cookie【这样做是为了方便用户跨站请求后跳转，到原来的站点时仍然保持登录态】
     因为这个功能在正常网站之间功能跳转时很常见的

    cookie攻击核心： cookie 最初被设计成了允许在第三方网站发起的请求中携带[点击其他网站内连接到自己以及登录过的网站的地址，登陆过的网站的cookie会自动带上]，CSRF 攻击就是利用了 cookie 的这一“弱点”
                    也就是第三方网站发起post请求【action设置为用户登陆过的站点】，cookie也会被带上,
                    
                    浏览器层防御：面对cookie的sameSite属性【服务端设置cookie的这个属性，告诉客户端这个cookie是 sameSite 还是Lax】，防御这个的：
                      ...
                      Lax：防御的状态如下：不发送cookie
                           ...
                           如果连接都预防了，那么任何第三方站点跳转本站，都没有cookie信息发送到服务端，也就没有登录态
                           这个就很不方便，Lax基本是统配；
                      不设置：不对sameSite设置的话，外部站点用url+参数跳转攻击肯定是没法防御
                              就连最危险的外部站点上post请求调教给目标站点，都没法防御，因为cookie会自动带过去
                    
                    服务器层面防御：因为有的浏览器不支持sameSite属性，所以通过referer判断请求来自哪里，是很有必要的
                                  也就是最最核心的数据操作，需要在服务器端添加一道referer判断的防御。
                                  涉及安全的请求都要用post；
                                  最最重要的一点，url带参数的请求，绝对不能涉及重要的数据操作！！！！
                                  因为sameSite设置为lax下，其他页面跳转url到自己站点发起请求
...
最终都会成为前端数据，而这个前端数据如何放到html，成了xss的关键：
  核心就一点：不要根据数据来拼接html字符串，然后把它设置到元素的innerHTML里面，这样是富文本输入，容易受到xss攻击
...
xss【跨站脚本攻击】防范从3方面：
              1.数据录入方面：前端过滤没啥用，可以随意伪造请求，
                需要后端对提交的数据根据不同业务类型进行格式的确认，比如电话号码，那就不能带字母
...
//cookie验证的坏处是，csrf攻击可能包含cookie导致通过，这个可以通过浏览器的cookie的sameSite设置拦截一大部分
//还有一部分可以通过referer来拦截；
...
function addToken ({phoneNum , IP , ua, id }) {
  ...
  //cookie中有csrf风险，而且不能跨域，但是可以防xss攻击；csrf风险可以通过设置cookie的sameSite为lax，以及服务端判断referer来鉴定
